...
! This file contains the routines that transpose data from Y to X pencil
  ...
  subroutine transpose_y_to_x_data(src, dst)
    ...
    ! Safety check
#ifdef DEBUG
    if (src%idir /= 2 .or. dst%idir /= 1 .or. &
...
#else    
       call mem_split_yx_complex(src%cvar, s1, s2, s3, work1_c, dims(1), &
            src%decomp%y1dist, src%decomp)
#endif
    else
#if defined(_GPU)
       call mem_split_yx_real(src%var, s1, s2, s3, work1_r_d, dims(1), &
            src%decomp%y1dist, src%decomp)
#else
       call mem_split_yx_real(src%var, s1, s2, s3, work1_r, dims(1), &
            src%decomp%y1dist, src%decomp)
#endif
    endif
    ...
    if (src%is_cplx) then
#ifdef EVEN
       call MPI_ALLTOALL(work1_c, src%decomp%y1count, &
            complex_type, work2_c, src%decomp%x1count, &
            complex_type, DECOMP_2D_COMM_COL, ierror)
       if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "MPI_ALLTOALL")
#else

#if defined(_GPU)
       call MPI_ALLTOALLV(work1_c_d, src%decomp%y1cnts, src%decomp%y1disp, &
            complex_type, work2_c_d, src%decomp%x1cnts, src%decomp%x1disp, &
            complex_type, DECOMP_2D_COMM_COL, ierror)
       if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "MPI_ALLTOALLV")
#else
       call MPI_ALLTOALLV(work1_c, src%decomp%y1cnts, src%decomp%y1disp, &
            complex_type, work2_c, src%decomp%x1cnts, src%decomp%x1disp, &
            complex_type, DECOMP_2D_COMM_COL, ierror)
       if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "MPI_ALLTOALLV")
#endif
...
#ifdef EVEN
       call MPI_ALLTOALL(work1_r, src%decomp%y1count, &
            real_type, work2_r, src%decomp%x1count, &
       ...
       do col_rank_id = 0, (col_comm_size - 1)
        nccl_stat = ncclSend(work1_r_d( src%decomp%y1disp(col_rank_id)+1 ), src%decomp%y1cnts(col_rank_id), ncclDouble, local_to_global_col(col_rank_id+1), nccl_comm_2decomp, cuda_stream_2decomp)
        nccl_stat = ncclRecv(work2_r_d( src%decomp%x1disp(col_rank_id)+1 ), src%decomp%x1cnts(col_rank_id), ncclDouble, local_to_global_col(col_rank_id+1), nccl_comm_2decomp, cuda_stream_2decomp)
       end do
       nccl_stat = ncclGroupEnd()
       cuda_stat = cudaStreamSynchronize(cuda_stream_2decomp)
#else
       call MPI_ALLTOALLV(work1_r_d, src%decomp%y1cnts, src%decomp%y1disp, &
            real_type, work2_r_d, src%decomp%x1cnts, src%decomp%x1disp, &
            real_type, DECOMP_2D_COMM_COL, ierror)
       if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "MPI_ALLTOALLV")
#endif
#else
       call MPI_ALLTOALLV(work1_r, src%decomp%y1cnts, src%decomp%y1disp, &
            real_type, work2_r, src%decomp%x1cnts, src%decomp%x1disp, &
            real_type, DECOMP_2D_COMM_COL, ierror)
       if (ierror /= 0) call decomp_2d_abort(__FILE__, __LINE__, ierror, "MPI_ALLTOALLV")
#endif
    ...
    if (src%is_cplx) then
#if defined(_GPU)
       call mem_merge_yx_complex(work2_c_d, d1, d2, d3, dst%cvar, dims(1), &
            src%decomp%x1dist, src%decomp)
#else
       call mem_merge_yx_complex(work2_c, d1, d2, d3, dst%cvar, dims(1), &
            src%decomp%x1dist, src%decomp)
#endif
    else
#if defined(_GPU)
       call mem_merge_yx_real(work2_r_d, d1, d2, d3, dst%var, dims(1), &
            src%decomp%x1dist, src%decomp)
#else
       call mem_merge_yx_real(work2_r, d1, d2, d3, dst%var, dims(1), &
            src%decomp%x1dist, src%decomp)
#endif
    endif
