...
! This file contains the routines that transpose data from Z to Y pencil
  ...
  subroutine transpose_z_to_y_data(src, dst, no_prepro)
    ...
    if (d2d_intranode) then
       do j = 1, decomp%zsz_loc(3)
          do k = 1, decomp%zsz_loc(1)
             out(decomp%intramap_split(k,j,4)) = in%var2d(k,j)
          enddo
       enddo
       return
    endif
  ...
  subroutine mem_split_zy_complex(in,n1,n2,n3,out,iproc,dist,decomp)
    ...
    if (d2d_intranode) then
       do j = 1, decomp%zsz_loc(3)
          do k = 1, decomp%zsz_loc(1)
             out(decomp%intramap_split(k,j,4)) = in%cvar2d(k,j)
          enddo
       enddo
       return
    endif
    ...
    if (d2d_intranode) then
       do k = 1, decomp%ysz_loc(3)
          do j = 1, decomp%ysz_loc(1)
             out%var2d(j,k) = in(decomp%intramap_merge(j,k,4))
          enddo
       enddo
       return
    endif
  ...
  subroutine mem_merge_zy_complex(in,n1,n2,n3,out,iproc,dist,decomp)
    ...
    if (d2d_intranode) then
       do k = 1, decomp%ysz_loc(3)
          do j = 1, decomp%ysz_loc(1)
             out%cvar2d(j,k) = in(decomp%intramap_merge(j,k,4))
          enddo
       enddo
       return
    endif
